...
def train(args, snapshot_path):
    ...
    model = SGODL(in_channel=1, out_channel=num_classes).cuda()
                ...
                if iter_num > 0 and iter_num % 200 == 0:
                    ...
                    if avg_metric[:, 0].mean() > best_performance:
                        ...
                        save_mode_path = os.path.join(snapshot_path,
                                                      'iter_{}_dice0_{}_dice1_{}_AveDice_{}.pth'.format(
                                                          iter_num, round(avg_metric[0, 0], 4), round(avg_metric[1, 0], 4), round(best_performance, 4)))
                    ...
                    logging.info(
                        'iteration %d : dice0 : %f dice1 : %f AveDice : %f hd95 : %f' % (iter_num, avg_metric[0, 0], avg_metric[1, 0], avg_metric[:, 0].mean(), avg_metric[:, 1].mean()))
                    model.train()
            else:
                if iter_num > 0 and iter_num % 200 == 0:
                    model.eval()
                    ...
                    logging.info(
                        ...
                        iter_num, avg_metric[0, 0].mean(), avg_metric[:, 1].mean()))
                    model.train()
...
if __name__ == "__main__":
    ...
    snapshot_path = "".format(args.exp, args.labeled_num)
    if not os.path.exists(snapshot_path):
        os.makedirs(snapshot_path)
    if os.path.exists(snapshot_path + '/my_code'):
        shutil.rmtree(snapshot_path + '/my_code')
    shutil.copytree('.', snapshot_path + '/my_code',
